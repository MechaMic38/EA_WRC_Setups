...
interface VehicleListboxProps {
    options: Vehicle[];
    selectedOption: Vehicle | null;
    placeholder?: string;
    error?: string | null;
    onChange: (option: Vehicle | null) => void;
}

export default function VehicleListbox({
    options,
    selectedOption,
                ...
                >
                    {selectedOption ? (
                        <div className="flex items-center gap-2">
                            <img
                                src={selectedOption.category.imgPath}
                                alt={`${selectedOption.manufacturer.name} ${selectedOption.name}`}
                                className="h-5 w-5 object-contain"
                            />
                            <span className="truncate">{`${selectedOption.manufacturer.name} ${selectedOption.name}`}</span>
                        </div>
                    ) : (
                        ...
                        {options.map((option) => (
                            ...
                            >
                                {({ selected }) => (
                                    <div className="flex items-center gap-2">
                                        <img
                                            src={option.category.imgPath}
                                            alt={`${option.manufacturer.name} ${option.name}`}
                                            className="h-5 w-5 object-contain"
                                        />
                                        <span
                                            className={`block truncate ${
