...
interface SurfaceTypeListboxProps {
    options: string[];
...
export default function SurfaceTypeListbox({
    options,
    selectedOption,
    placeholder = "Select a surface type",
    error = null,
    onChange,
}: SurfaceTypeListboxProps) {
    return (
                ...
                >
                    {selectedOption ? (
                        <div className="flex items-center gap-2">
                            {
                                SURFACE_TYPES_MAP[
                                    selectedOption as keyof typeof SURFACE_TYPES_MAP
                                ]?.icon
                            }
                            <span>
                                {
                            ...
                            >
                                {({ selected }) => (
                                    <div className="flex items-center gap-2">
                                        {
                                            SURFACE_TYPES_MAP[
                                                option as keyof typeof SURFACE_TYPES_MAP
                                            ]?.icon
                                        }
                                        <span
                                            className={`block truncate ${
