import { SEASONS_MAP, SURFACE_CONDITIONS_MAP } from "@/constants";
import {
    Listbox,
    ListboxButton,
    ListboxOption,
...
interface SurfaceConditionListboxProps {
    options: string[];
...
export default function SurfaceConditionListbox({
    options,
    selectedOption,
    placeholder = "Select a condition",
    error = null,
    onChange,
}: SurfaceConditionListboxProps) {
    return (
                        ...
                        <div className="flex items-center gap-2">
                            {
                                SURFACE_CONDITIONS_MAP[
                                    selectedOption as keyof typeof SURFACE_CONDITIONS_MAP
                                ]?.icon
                            }
                            <span>
                                {
                            ...
                            >
                                {({ selected }) => (
                                    <div className="flex items-center gap-2">
                                        {
                                            SURFACE_CONDITIONS_MAP[
                                                option as keyof typeof SURFACE_CONDITIONS_MAP
                                            ]?.icon
                                        }
                                        <span
                                            className={`block truncate ${
