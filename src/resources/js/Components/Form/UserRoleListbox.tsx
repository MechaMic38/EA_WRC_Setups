...
interface UserRoleListboxProps {
    options: string[];
...
export default function UserRoleListbox({
    options,
    selectedOption,
    placeholder = "Select a role",
    error = null,
    onChange,
}: UserRoleListboxProps) {
    return (
                ...
                >
                    {selectedOption ? (
                        <div className="flex items-center gap-2">
                            {
                                USER_ROLES_MAP[
                                    selectedOption as keyof typeof USER_ROLES_MAP
                                ]?.icon
                            }
                            <span>
                                {
                            ...
                            >
                                {({ selected }) => (
                                    <div className="flex items-center gap-2">
                                        {
                                            USER_ROLES_MAP[
                                                option as keyof typeof USER_ROLES_MAP
                                            ]?.icon
                                        }
                                        <span
                                            className={`block truncate ${
